...
char	*combine_subs(char *input, int start, int end)
{
	...
	tmp1 = ft_substr(input, 0, start);	// Part before the start index
	tmp2 = ft_substr(input, start + 1, end - start - 1);// Part between start and end
	tmp3 = ft_strdup(input + end + 1);// Part after the end index
	free(input);// Free the original input string to avoid memory leaks.
	input = NULL;
	input = ft_strjoin_con(tmp1, tmp2, tmp3);// Combine the three parts into a new string.
	free(tmp1);
	free(tmp2);
	free(tmp3);
...
char	*is_quote(char *input, int *i)
{
	...
	input = combine_subs(input, start, *i); // Call combine_subs to process the substring.
	return (input); // modified input string.
}

/**
 * function processes double quotes in the input string. It handles dollar signs 
 * within the quotes, updating the iterator as it finds these characters. It calls 
...
char	*is_double_quote(char *input, int *i, t_env *env_dup)
{
	...
	while (input[++(*i)])	// Increment the index until a closing double quote is found.
	{
		if (input[*i] == '$') // Check for dollar sign for variable substitution.
